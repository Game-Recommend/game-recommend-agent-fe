...
충분하므로 배지를 달지 않습니다. 반복이 생기는 경로는 셋입니다. 추천 후보 중 가격·사양을 조회하지 않았거나
조회가 실패한 게임은 백엔드 러너의 안전망이 다시 부르고(가격·하드웨어, 에이전트 추론 칸 안), 확정 후보 중 리뷰
요약이 없는 게임은 후처리가 다시 부르며(리뷰 요약, 조건 판정 뒤 미디어와 병렬), 초안이 거부되면 에이전트가 한 번
더 돕니다. 게임 검색과 리뷰 점수는 안전망이 없어 프롬프트가 재호출을 막는 대로 보통 한 번씩만 돕니다.
...
어긋납니다. 그래서 도구 가짓수인 5와도 다를 수 있습니다. 리뷰 점수를 고르지 않으면 4회고, 안전망이 돌거나
초안이 거부돼 다시 부르면 5회를 넘습니다.
